...
def append_row(folder_name, alpha):
    ...
    f"{round(((jobs_data_0 + jobs_data_1 + jobs_data_2) / (n_jobs_0 + n_jobs_1 + n_jobs_2)) * 100, 2):.2f}%",  # Deadline ratio rounded to 2 decimal places and converted to percentage
    f"{round(jobs_data_0 / n_jobs_0 * 100, 2):.2f}%",  # Deadline ratio for job type 0 rounded to 2 decimal places and converted to percentage
    f"{round(jobs_data_1/ n_jobs_1 * 100, 2):.2f}%",  # Deadline ratio for job type 1 rounded to 2 decimal places and converted to percentage
    f"{round(jobs_data_2 / n_jobs_2 * 100, 2):.2f}%"   # Deadline ratio for job type 2 rounded to 2 decimal places and converted to percentage
    f"{ts[0]}"
    ]
...
file_path = "table_data_WORKERS_OR_CLOUD.txt"
