...
def run_simulation(alpha):
    session_id = f'{SESSION_ID}_{alpha:.2f}_RECHARGE'
    ...
    nodes.append(create_node(env, 1, 0, Node.NodeType.WORKER, 1.0, 9, alpha, solar_panel_spec_by_node_id.get(1),SOLAR_PANEL_ENABLED))
    nodes.append(create_node(env, 2, 0, Node.NodeType.WORKER, 0.9, 8, alpha , solar_panel_spec_by_node_id.get(2),SOLAR_PANEL_ENABLED))
    nodes.append(create_node(env, 3, 0, Node.NodeType.WORKER, 0.6, 7, alpha, solar_panel_spec_by_node_id.get(3),SOLAR_PANEL_ENABLED))
...
def create_node(env, node_id, belong_to_cluster_id, node_type, machine_speed,batt, alpha, node_solar_panel_spec=None, solar_panel_enabled=False):
    return Node(env,
                node_id,
                SESSION_ID,
...
def gen_spec_solar_panels(n_nodes, simulation_time, start_date_str, latitude=41.80, longitude=12.36, altitude=5.0,
    ...
    for i in range(n_nodes):
        if latitude_list is not None and longitude_list is not None and altitude_list is not None:
            panels_specs.append(SolarPanelSpec(node_id=i + 1, latitude=latitude_list[i], longitude=longitude_list[i],
            ...
            panels_specs.append(SolarPanelSpec(node_id=i + 1, latitude=latitude, longitude=longitude, altitude=altitude,
